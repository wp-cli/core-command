Feature: Update WordPress core
  ...
  Scenario: Update from a ZIP file
    ...
    When I run `curl -sS -L -o wordpress-6.2.zip http://wordpress.org/wordpress-6.2.zip`
  ...
  Scenario: Output in JSON format
    ...
    When I run `curl -sS -L -o wordpress-6.8.zip http://wordpress.org/wordpress-6.8.zip`
